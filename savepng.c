...
int SDL_SavePNG_RW(SDL_Surface *surface, SDL_RWops *dst, int freedst) 
{
        ...
        && (pal = surface->format->palette))
    {
        ...
        pal_ptr = (png_colorp)malloc((uint32_t) pal->ncolors * sizeof(png_color));
    ...
    if (dpi > 0)
    {
        ...
        ppm_y = ppm_x;
        png_set_pHYs (png_ptr, info_ptr, (png_uint_32) ppm_x, (png_uint_32) ppm_y, PNG_RESOLUTION_METER);
    }

    png_set_IHDR (png_ptr, info_ptr, (png_uint_32) surface->w, (png_uint_32) surface->h, 8, colortype,
...
#ifdef USE_ROW_POINTERS
    row_pointers = (png_bytep*) malloc(sizeof(png_bytep)*(uint32_t) surface->h);
